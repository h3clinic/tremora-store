"""Exact token classification for Ego4D normalized IMU CSVs.

The preserved source token and the parsed number are different facts, and the
parser keeps both.  Python's :func:`float` would accept ``1_000`` and store a
...
def classify(token: str) -> tuple[TokenKind, float | None]:
    ...
    Surrounding whitespace is not stripped: a normalized Ego4D CSV is machine
