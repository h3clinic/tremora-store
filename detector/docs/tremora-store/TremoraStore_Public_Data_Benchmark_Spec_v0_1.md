...
**PADS-P0.4 rate ablation and anti-aliasing is `PASS`.** All eighteen conditions are satisfied: 38,316 of 39,840 rate-windows eligible at 100, 50, 30 and 25 Hz, carrying 76,632 derived spectra over 7,981,740 samples all placed on exact rational time — 30 Hz has no exact picosecond period and its ordinals are held as exactly *k*/30 s rather than rounded. Resampling is two explicit stages, bracketing onto a 100 Hz parent grid and then one frozen linear-phase FIR per rate, and support is their intersection in that order: 730,804 output ordinals were removed by the parent stage that the filter guard alone would have admitted, and 0 survived over an interval the source could not bracket. Nothing is padded or renormalized at an edge. Across 469 participants the 3–10 Hz core band is preserved identically at every rate — power ratio 0.9710, 0.9712, 0.9712, 0.9712 — while the accepted 25 Hz loss is confined to the 10–12 Hz edge, where power falls to 0.44 and shape distance rises from 0.008 to 0.33. Source-direct and replay-derived paths agree exactly across 5,627 comparisons, maximum bin error 0.0. Two processes produced the identical evidence hash `2aaebd34…`. See [`pads_p04_release_audit.json`](../../benchmarks/pads_p04_release_audit.json).
...
| PADS-P0.4 rate-ablation and anti-aliasing audit | `PASS_PADS_RATE_ABLATION_AND_ANTI_ALIASING`; 18/18, 76,632 derived spectra at 100/50/30/25 Hz, 3–10 Hz core preserved at every rate, 25 Hz loss confined to the 10–12 Hz edge, maximum bin error 0.0 |
